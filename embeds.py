...
def help(name):
    ...
    helpEmbed.add_field(name="",value="`member gban` メンバーをグローバルBANします",inline=False)
...
def managelog(target,executer,method,reason):
    logEmbed = discord.Embed(title=f"{target}を{method}しました",color=discord.Color.red(),timestamp=datetime.now())
    logEmbed.add_field(name="担当者",value=executer)
    logEmbed.add_field(name="理由",value=f"`{reason}`")
    logEmbed.set_footer(text=ctx.guild.name)
